...
@interface HLCropView () <UIScrollViewDelegate, UIGestureRecognizerDelegate>

@property (nonatomic) UIScrollView *scrollView;
@property (nonatomic) UIView *zoomingView;
@property (nonatomic) UIImageView *imageView;

@property (nonatomic) HLCropRectView *cropRectView;
...
//        [self.scrollView addGestureRecognizer:rotationGestureRecognizer];
        ...
        self.cropRectView = [[HLCropRectView alloc] init];
...
- (CGRect)cappedCropRectInImageRectWithCropRectView:(HLCropRectView *)cropRectView
{
...
- (void)zoomToCropRect:(CGRect)toRect
{
    ...
    if (CGRectEqualToRect(self.scrollView.frame, toRect)) {
        ...
        //初始化进来 默认正方形
        CGRect cropRect1 = self.scrollView.frame;
        CGFloat width1 = CGRectGetWidth(cropRect1);
        CGFloat height1 = CGRectGetHeight(cropRect1);
        if (width1 < height1) {
            width1 = height1 * 1;
        } else {
            height1 = width1 * 1;
        }
        cropRect1.size = CGSizeMake(width1, height1);
        toRect = cropRect1;
    }
